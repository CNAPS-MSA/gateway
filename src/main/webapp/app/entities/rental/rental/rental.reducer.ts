...
import { IRental, defaultValue } from 'app/shared/model/rental/rental.model';
...
const apiUrl = 'services/rental/api/rentals';
...
export const getEntities: ICrudGetAllAction<IRental> = (page, size, sort) => {
  ...
  return {
    ...
    payload: axios.get<IRental>(`${requestUrl}${sort ? '&' : '?'}cacheBuster=${new Date().getTime()}`)
  };
};
